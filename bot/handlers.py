"""
bot/handlers.py — All Telegram command and conversation handlers.

Conversation flow (Forex only — crypto removed):
  /trade → Balance → Pair → Trade Style → Risk → Lot Size → Notes → Generate

Natural language:
  "I have $500, analyse EUR/USD, aggressive risk, scalp"
"""

import logging
import re
from typing import Optional

from telegram import Update, Message
from telegram.constants import ChatAction
from telegram.ext import (
    ContextTypes,
    ConversationHandler,
    CommandHandler,
    MessageHandler,
    CallbackQueryHandler,
    filters,
)

from ai.analyst import get_trade_plan, AnalystError
from bot.formatter import format_trade_plan, format_history_entry
from bot.keyboards import (
    trade_style_keyboard,
    risk_keyboard,
    forex_pair_keyboard,
    lot_size_keyboard,
    cancel_keyboard,
)
from db.database import get_user, upsert_user, log_trade, get_trade_history

logger = logging.getLogger(__name__)

# ── Conversation states ───────────────────────────────────────────────────────

(
    ASK_BALANCE,
    ASK_PAIR,
    ASK_STYLE,
    ASK_RISK,
    ASK_LOT_SIZE,
    ASK_NOTES,
) = range(6)

TRADE_KEY = "trade_data"


# ── /start ────────────────────────────────────────────────────────────────────

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    await upsert_user(user.id, user.username or "")
    await update.message.reply_text(
        f"👋 Welcome, {user.first_name}!\n\n"
        "I'm your *AI Forex Trade Planner* — powered by live market data and Claude AI.\n\n"
        "I analyse the Forex market in real time and return a complete trade plan:\n"
        "• Entry, Stop Loss, Take Profits — anchored to live price\n"
        "• ATR-validated stop loss (not arbitrary pips)\n"
        "• Previous Day High/Low as liquidity targets\n"
        "• ICT/SMC confluence scoring (weighted)\n"
        "• Kill zone timing awareness\n"
        "• Mathematically exact risk and profit calculations\n\n"
        "Commands:\n"
        "/trade — Start a new trade analysis\n"
        "/history — View your last 5 trade plans\n"
        "/setbalance — Save your default balance\n"
        "/help — Show this message\n\n"
        "⚠️ *Advisory only. Always verify before placing any trade.*",
        parse_mode="Markdown",
    )


async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await start(update, context)


# ── /setbalance ───────────────────────────────────────────────────────────────

async def set_balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    args = context.args
    if not args:
        await update.message.reply_text("Usage: `/setbalance 500`", parse_mode="Markdown")
        return
    try:
        balance = float(args[0].replace(",", "").replace("$", ""))
        if balance <= 0:
            raise ValueError
        user_id = update.effective_user.id
        await upsert_user(user_id, update.effective_user.username or "", default_balance=balance)
        await update.message.reply_text(
            f"✅ Default balance saved: *${balance:,.2f}*", parse_mode="Markdown"
        )
    except (ValueError, IndexError):
        await update.message.reply_text(
            "❌ Invalid amount. Use: `/setbalance 500`", parse_mode="Markdown"
        )


# ── /history ──────────────────────────────────────────────────────────────────

async def history(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.effective_user.id
    trades = await get_trade_history(user_id, limit=5)
    if not trades:
        await update.message.reply_text("📭 No trade history yet. Use /trade to get started.")
        return
    lines = ["📋 *Your last trade plans:*\n"]
    for i, trade in enumerate(trades, 1):
        lines.append(format_history_entry(trade, i))
    await update.message.reply_text("\n\n".join(lines), parse_mode="Markdown")


# ── /trade — conversation entry ───────────────────────────────────────────────

async def trade_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data[TRADE_KEY] = {}
    user = await get_user(update.effective_user.id)
    default_balance = user.get("default_balance") if user else None

    if default_balance:
        context.user_data[TRADE_KEY]["balance"] = default_balance
        await update.message.reply_text(
            f"💰 Using saved balance: *${default_balance:,.2f}*\n\n"
            f"🔀 Which *Forex pair* do you want to trade?",
            parse_mode="Markdown",
            reply_markup=forex_pair_keyboard(),
        )
        return ASK_PAIR

    await update.message.reply_text(
        "💰 What is your *account balance* in USD?\n\n_Example: 500 or 1000_",
        parse_mode="Markdown",
        reply_markup=cancel_keyboard(),
    )
    return ASK_BALANCE


async def received_balance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip().replace(",", "").replace("$", "")
    try:
        balance = float(text)
        if balance <= 0:
            raise ValueError
        context.user_data[TRADE_KEY]["balance"] = balance
        await update.message.reply_text(
            f"✅ Balance: *${balance:,.2f}*\n\n🔀 Which *Forex pair*?",
            parse_mode="Markdown",
            reply_markup=forex_pair_keyboard(),
        )
        return ASK_PAIR
    except ValueError:
        await update.message.reply_text(
            "❌ Invalid. Enter a number like `500` or `1000`.",
            parse_mode="Markdown",
        )
        return ASK_BALANCE


async def received_pair(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()

    if query.data == "pair_auto":
        context.user_data[TRADE_KEY]["pair"] = None
        pair_label = "🤖 AI selects best pair"
    elif query.data == "pair_custom":
        await query.edit_message_text(
            "✏️ Type your Forex pair (e.g. `XAU/USD`, `EUR/GBP`, `GBP/JPY`):",
            parse_mode="Markdown",
        )
        return ASK_PAIR
    else:
        raw = query.data.replace("pair_", "")
        if "/" not in raw:
            if len(raw) == 6:
                raw = raw[:3] + "/" + raw[3:]
            elif len(raw) == 7:
                raw = raw[:4] + "/" + raw[4:]
        context.user_data[TRADE_KEY]["pair"] = raw
        pair_label = raw

    await query.edit_message_text(
        f"✅ Pair: *{pair_label}*\n\n⚡ *Trade style?*",
        parse_mode="Markdown",
        reply_markup=trade_style_keyboard(),
    )
    return ASK_STYLE


async def received_pair_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip().upper().replace(" ", "")
    if "/" not in text:
        if len(text) == 6:
            text = text[:3] + "/" + text[3:]
        elif len(text) == 7:
            text = text[:4] + "/" + text[4:]
    context.user_data[TRADE_KEY]["pair"] = text
    await update.message.reply_text(
        f"✅ Pair: *{text}*\n\n⚡ *Trade style?*",
        parse_mode="Markdown",
        reply_markup=trade_style_keyboard(),
    )
    return ASK_STYLE


async def received_style(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    style = "scalp" if query.data == "style_scalp" else "swing"
    context.user_data[TRADE_KEY]["style"] = style
    label = "⚡ Scalp" if style == "scalp" else "📈 Swing"
    await query.edit_message_text(
        f"✅ Style: *{label}*\n\n⚖️ *Risk appetite?*",
        parse_mode="Markdown",
        reply_markup=risk_keyboard(),
    )
    return ASK_RISK


async def received_risk(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    risk_map = {
        "risk_conservative": "conservative",
        "risk_moderate": "moderate",
        "risk_aggressive": "aggressive",
    }
    risk = risk_map.get(query.data)
    if not risk:
        return ASK_RISK
    context.user_data[TRADE_KEY]["risk"] = risk
    risk_emoji = {"conservative": "🛡", "moderate": "⚖️", "aggressive": "🔥"}
    await query.edit_message_text(
        f"✅ Risk: *{risk_emoji[risk]} {risk.capitalize()}*\n\n"
        f"📐 *Lot size?*\n_0.01 is the minimum on most brokers._",
        parse_mode="Markdown",
        reply_markup=lot_size_keyboard(),
    )
    return ASK_LOT_SIZE


async def received_lot_size_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    if query.data == "lot_custom":
        await query.edit_message_text(
            "✏️ Type your lot size (e.g. `0.03`, `0.15`, `2.00`):",
            parse_mode="Markdown",
        )
        return ASK_LOT_SIZE
    lot = query.data.replace("lot_", "")
    context.user_data[TRADE_KEY]["lot"] = lot
    await query.edit_message_text(
        f"✅ Lot size: *{lot}*\n\n📝 Any *notes*? (e.g. 'bearish on DXY') or type *skip*",
        parse_mode="Markdown",
    )
    return ASK_NOTES


async def received_lot_size_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    try:
        lot = float(text)
        if lot < 0.01:
            await update.message.reply_text(
                "⚠️ Minimum lot size is *0.01*.", parse_mode="Markdown"
            )
            return ASK_LOT_SIZE
        lot_str = f"{lot:.2f}"
        context.user_data[TRADE_KEY]["lot"] = lot_str
        await update.message.reply_text(
            f"✅ Lot: *{lot_str}*\n\n📝 Any *notes*? Or type *skip*.",
            parse_mode="Markdown",
        )
        return ASK_NOTES
    except ValueError:
        await update.message.reply_text(
            "❌ Invalid. Enter a number like `0.01`, `0.10`, or `1.00`.",
            parse_mode="Markdown",
        )
        return ASK_LOT_SIZE


async def received_notes(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    notes = "" if text.lower() == "skip" else text
    context.user_data[TRADE_KEY]["notes"] = notes
    return await _generate_and_send(update.message, context)


# ── Natural language handler ──────────────────────────────────────────────────

def _parse_nl(text: str) -> Optional[dict]:
    tl = text.lower()
    m = re.search(r'\$?([\d,]+(?:\.\d{1,2})?)', tl)
    if not m:
        return None
    try:
        balance = float(m.group(1).replace(",", ""))
    except ValueError:
        return None
    if balance <= 0:
        return None

    pm = re.search(r'\b([A-Za-z]{2,4}[/]?[A-Za-z]{2,4})\b', text)
    pair = None
    if pm:
        raw = pm.group(1).upper()
        skip = {"HAVE", "WANT", "NEED", "WITH", "FOR", "AND", "THE"}
        if raw not in skip and len(raw) >= 5:
            if "/" not in raw and len(raw) == 6:
                raw = raw[:3] + "/" + raw[3:]
            pair = raw

    risk = (
        "conservative" if any(w in tl for w in ["conservative", "low risk", "safe"])
        else "aggressive" if any(w in tl for w in ["aggressive", "high risk"])
        else "moderate"
    )
    style = "scalp" if any(w in tl for w in ["scalp", "quick", "fast"]) else "swing"

    return {"balance": balance, "pair": pair, "risk": risk, "style": style,
            "lot": "0.01", "notes": ""}


async def handle_natural_language(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    text = update.message.text.strip()
    parsed = _parse_nl(text)
    if not parsed:
        await update.message.reply_text(
            'Try: _"I have $500, analyse EUR/USD, aggressive risk, scalp"_\n'
            "Or use /trade to go step by step.",
            parse_mode="Markdown",
        )
        return
    context.user_data[TRADE_KEY] = parsed
    pair_label = parsed["pair"] or "AI picks best pair"
    await update.message.reply_text(
        f"📋 Balance: ${parsed['balance']:,.2f} | Pair: {pair_label} | "
        f"Risk: {parsed['risk'].capitalize()} | Style: {parsed['style'].capitalize()}\n\n"
        "⏳ Fetching live data…",
        parse_mode="Markdown",
    )
    await _generate_and_send(update.message, context)


# ── Trade generation ──────────────────────────────────────────────────────────

async def _generate_and_send(
    message: Message, context: ContextTypes.DEFAULT_TYPE
) -> int:
    d        = context.user_data.get(TRADE_KEY, {})
    balance  = d.get("balance", 0)
    pair     = d.get("pair")
    risk     = d.get("risk", "moderate")
    style    = d.get("style", "swing")
    lot      = d.get("lot", "0.01")
    notes    = d.get("notes", "")
    user_id  = message.from_user.id

    if not balance or balance <= 0:
        await message.reply_text("⚠️ Balance missing. Use /trade to start again.")
        return ConversationHandler.END

    thinking = await message.reply_text("⏳ Fetching live data and building analysis…")
    await message.chat.send_action(ChatAction.TYPING)

    try:
        plan = await get_trade_plan(
            balance=balance, pair=pair, risk=risk,
            style=style, notes=notes, lot=lot,
        )
    except AnalystError as exc:
        await thinking.delete()
        await message.reply_text(
            f"❌ Analysis failed: {exc}\n\nTry again with /trade.",
        )
        return ConversationHandler.END
    except Exception:
        logger.exception("Unexpected error in generate_and_send")
        await thinking.delete()
        await message.reply_text("❌ Unexpected error. Please try /trade again.")
        return ConversationHandler.END

    # Log to DB regardless of TRADE or NO_TRADE
    try:
        # Map v3 schema fields to DB columns for consistency
        db_plan = {
            "pair":             plan.get("pair"),
            "direction":        plan.get("direction", "N/A"),
            "entry":            plan.get("entry", "N/A"),
            "stop_loss":        plan.get("sl", "N/A"),
            "risk_amount":      plan.get("risk_usd", "N/A"),
            "confluence_score": plan.get("confluence", 0),
            **plan,
        }
        await log_trade(user_id, db_plan)
    except Exception:
        logger.exception("Failed to log trade")

    await thinking.delete()
    await message.reply_text(
        format_trade_plan(plan, balance),
        parse_mode="Markdown",
    )

    context.user_data.pop(TRADE_KEY, None)
    return ConversationHandler.END


# ── Cancel ────────────────────────────────────────────────────────────────────

async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data.pop(TRADE_KEY, None)
    if update.callback_query:
        await update.callback_query.answer()
        await update.callback_query.edit_message_text("❌ Trade analysis cancelled.")
    else:
        await update.message.reply_text("❌ Cancelled. Use /trade to start again.")
    return ConversationHandler.END


# ── Register all handlers ─────────────────────────────────────────────────────

def register_handlers(app) -> None:
    app.add_handler(CommandHandler("start", start))
    app.add_handler(CommandHandler("help", help_command))
    app.add_handler(CommandHandler("setbalance", set_balance))
    app.add_handler(CommandHandler("history", history))

    trade_conv = ConversationHandler(
        entry_points=[CommandHandler("trade", trade_start)],
        states={
            ASK_BALANCE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, received_balance)
            ],
            ASK_PAIR: [
                CallbackQueryHandler(received_pair, pattern="^pair_"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, received_pair_text),
            ],
            ASK_STYLE: [
                CallbackQueryHandler(received_style, pattern="^style_")
            ],
            ASK_RISK: [
                CallbackQueryHandler(received_risk, pattern="^risk_")
            ],
            ASK_LOT_SIZE: [
                CallbackQueryHandler(received_lot_size_callback, pattern="^lot_"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, received_lot_size_text),
            ],
            ASK_NOTES: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, received_notes)
            ],
        },
        fallbacks=[
            CallbackQueryHandler(cancel, pattern="^cancel$"),
            CommandHandler("cancel", cancel),
        ],
        allow_reentry=True,
        per_message=False,
    )
    app.add_handler(trade_conv)

    app.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, handle_natural_language)
    )
